...
import AddAndEditProject from "./AddAndEditProject";
...
export default function ProjectManagement() {
  ...
  return (
    <div className="p-6 flex flex-col items-center">
          ...
          <tbody>
            {projects.map((p) => (
              <tr key={p.id} className="border-t border-gray-300">
                ...
                <td className="px-4 py-2 border-r border-gray-300">{p.name}</td>
                <td className="px-4 py-2 flex justify-center space-x-2">
                  <button className="bg-yellow-400 text-black px-3 py-1 rounded w-20 hover:bg-yellow-500"
                    onClick={() => setIsOpen(true)}
                  >
                    Sửa
                  </button>
                  <button className="bg-red-500 text-white px-3 py-1 rounded w-20 hover:bg-red-600"
                    onClick={() => setIsDeleteOpen(true)}
                  >
                    Xóa
                  </button>
                  <button className="bg-blue-500 text-white px-3 py-1 rounded w-20 hover:bg-blue-600" 
                    onClick={() => {navi("/project-detail")}}
                  >
                    Chi tiết
                  </button>
                </td>
      ...
      <AddAndEditProject isOpen={isOpen} onClose={() => setIsOpen(false)}></AddAndEditProject>
      <DeleteProject isOpen={isDeleteOpen} onClose={() => setIsDeleteOpen(false)}></DeleteProject>
    </div>
  );
}
